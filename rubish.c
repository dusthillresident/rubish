...
struct item  getValue( struct interp *interp, char **p ){
 ...
 switch( item.type ){
  ...
  case RPAREN: case RBRACKET: case STOP:
   return NOTHINGITEMOFTYPE( item.type );
  ...
  case LPAREN: {
   ...
   if( item.type == NOTHING ){
    if( item.data.integer == RPAREN ) return UNDEFINEDITEM;
    interp->errorMessage = item.data.integer == STOP ? "syntax error (note: you can't use ';' inside an expression)" : "syntax error";
    return ERRORITEM(*p);
   }
...
struct item  eval_p( struct interp *interp, char **p, int inParens ){
 struct item  value;  value.type = UNDEFINED;
 struct item  nextItem;
 while(1){
  nextItem = getValue( interp, p );
  if( nextItem.type == ERROR ){  deleteItem( &value );  return nextItem;  }
  if( nextItem.type == NOTHING ){
   switch( nextItem.data.integer ){
    case RBRACKET:  deleteItem( &value );  interp->errorMessage = "syntax error: extraneous ']'";  return ERRORITEM(*p);
    case RPAREN:
   ...
   deleteItem( &value );  value = nextItem;
  }
 }
}
